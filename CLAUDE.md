...
5. **端子番号には必ず `source`（出典）と `verified` を持たせる。** 未検証の型番を検証済みとして扱わない。MY2N / MY4N / MY4N-D2 は OMRON 公式データシート（J199）と照合済みで `verified: true`（`design.md` §4.4）。**新しい型番を足すときは `verified: false` から始める。** 特に MY2N1 / MY4N1 など末尾に「1」が付く型番はコイルの極性が逆（Type 2）なので、既存の端子表を流用しない。
