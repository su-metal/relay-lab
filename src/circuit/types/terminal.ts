...
export type TerminalDefinition = {
  ...
  side: TerminalSide;
  /**
   * 未接続でも警告しない端子（design.md §3.1・§4.15）。
   *
   * **端子が多い機器のための逃げ道。** 46 端子の調光コントローラでは、
   * 使う端子のほうが少ないのが普通で、未接続をすべて指摘すると
   * **本当に挿し忘れている 1 本が 40 本の雑音に埋もれる**（§5.7）。
   *
   * 立てるのは「使わないことが正常」な端子だけ —— 予備の GND、
   * 使わない出力回路、`No Connect` と印字された端子など。
   * **繋がないと機器が働かない端子には絶対に立てない。** 電源も、
   * 調光ランプの調光信号（挿し忘れると全灯する）も対象外。
   */
  optional?: boolean;
};
