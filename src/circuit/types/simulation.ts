...
export type SimulationInput = {
  ...
  /**
   * 直前のフェード状態（前回の `SimulationResult.fades`・design.md §5.18）。
   *
   * `previousTimers` とまったく同じ性質で、**渡し忘れると電圧が動かない** ——
   * 毎回「今この瞬間に目標へ到達した」ところからやり直すので、
   * 調光出力が設定を変えた瞬間に飛ぶ（フェードが無かった頃と同じ挙動になる）。
   */
  previousFades?: ReadonlyMap<string, FadeState>;
  /**
...
/**
 * 調光出力 1 チャンネルのフェード状態（design.md §5.18）。
 * キーは `fadeKey()`（`componentId:channelId`）。
 *
 * **今出している電圧は持たない。** `TimerState` が出力（接点が動いているか）を
 * 持たないのとまったく同じ理由で、目標・開始値・開始時刻から必ず導けるものを
 * 別に持つと片方だけ更新されてずれる。導出は `engine/fade.ts` の
 * `fadeVoltsOf()` 1 箇所に置く。
 */
export type FadeState = {
  /** 目標電圧（V）。インスタンスの `channelVolts` から決まる */
  targetVolts: number;
  /**
   * ランプを開始したときの電圧（V）。
   *
   * **「前の目標」ではなく「打ち直した瞬間の実効電圧」を入れる。** 前の目標を
   * 入れると、**フェードの途中で設定を動かした瞬間に電圧が飛ぶ。**
   */
  fromVolts: number;
  /**
   * `targetVolts` が今の値になった時刻（ms）。**`null` は「開始からずっとこの値」。**
   *
   * `TimerState.changedAtMs` と同じ約束で、経過時間 ∞ と読む ——
   * 「とっくに到達済み」が特別扱い無しに出る。**0 で初期化してはいけない** ——
   * 開始直後の調光出力が 0V から目標へ這い上がることになり、
   * 実機（電源を入れた時点で設定値を出している）と食い違う。
   */
  changedAtMs: number | null;
};
...
export type SimulationResult = {
  ...
  /**
   * 調光出力の `fadeKey()` → フェード状態。次回の `previousFades` になる
   * （design.md §5.18）。フェードを持たない回路では空。
   */
  fades: ReadonlyMap<string, FadeState>;
  /**
   * **次に結果が変わり終わる時刻（ms）。** 動いているものが 1 つも無ければ
   * `undefined`。
   *
   * 入るのは 2 種類（design.md §5.13・§5.18）。
   *
   * - カウント中のタイマー … 接点が変わる時刻。**離散**（その瞬間だけ変わる）
   * - ランプ中の調光出力 … フェードが終わる時刻。**連続**（そこまで毎瞬変わる）
   *
   * **ストアが「まだ時計を進める必要があるか」を判断する唯一の手がかり。**
   * これが無いと、タイマーも調光出力も置いていない回路でも延々と再計算を
   * 回し続けることになる。
   *
   * **連続と離散を混ぜてよいのは、ストアが刻みを自分で決めているため。**
   * `simulationStore` はこの値を「まだ動いているか」の 1 ビットとしか読まず、
   * 刻みは固定の 50ms。だからフェードは「終わる時刻」を返すだけでよく、
   * 途中の値は 50ms ごとの解き直しで `fadeVoltsOf()` が出す。
   * **到達済みで `undefined` を返すのは両方に共通の約束** —— 返し続けると
   * ストアが時計を止められない。
   */
