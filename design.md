...
`DELETE_KEYS` / `FLIP_KEYS` / `ARRANGE_KEYS` / `PAN_ACTIVATION_KEY` / `PAN_BUTTONS` /
`MULTI_SELECT_KEYS` は `lib/shortcuts.ts` に集約し、**React Flow へ渡す値も
`useFlipShortcut` / `useArrangeShortcut` の判定も、ヘルプの表も同じ定数から作る。**
