...
利用者から提供された Panasonic『液晶プロジェクター PT-VX430J』仕様書（VX430J_STM_01・2021/01/14 作成・1/8〜8/8）で確認した事実は次の 2 点。

- 使用電源は AC100V 50Hz/60Hz（■機器仕様）
- 接続端子は HDMI・コンピューター(RGB)・ビデオ・音声・**シリアル入力端子（D-Sub 9P・メス型・RS-232C 準拠、外部制御用）**・LAN・USB のみ（■接続端子リスト・2/8 ページ）。**無電圧接点で開閉できる制御端子は無い**

この盤の他の実機（調光操作卓・調光コントローラ）と違い、PT-VX430J は電源 ON/OFF をリレー回路へ直接組み込める形で持っていない。そこでプロジェクター本体は `kind: "lamp"` の AC100V 負荷（L/N の 2 端子）として表し、**電源 ON/OFF の概念そのものを持たせない** —— 実際の投影 ON/OFF はシリアルコマンドで行うもので、本アプリは通信の中身を扱わない（§6）。シリアル入力端子は物理的な実在を示すためだけの端子として持たせるが、ピン配列（TX/RX/GND）は仕様書に無く、接続の中身も判定しない。

AC100V 電源とシリアル入力端子はどちらもプラグ／コネクタ接続で、ねじ端子のような印字された番号・記号を持たない。したがって確認できたのは「端子構成の事実」であって「実端子番号」ではなく、**検証対象そのものが無いため `verified: false`**（§4.4 の考え方と同じ）。

#### VP コントローラー（実機を持たない仮の中継機器）

PT-VX430J のように無電圧接点の制御端子を持たないプロジェクターへ、調光コントローラ等の開放コレクタ出力（無電圧接点相当）からの ON/OFF をシリアルコマンドへ変換して送る、市販の RS-232C 接続ボックス相当を一般化した部品。**実機の製品ではなく本アプリのための仮の中継機器**であることを明記したうえで追加した（ユーザー判断）。

コイル 1 個（A1/A2・IEC 60947-1 / EN 50005 の慣例記号）と、プロジェクターのシリアル入力端子へつなぐための飾りの端子だけを持ち、**接点は 1 個も持たない**。実機の変換の中身（RS-232C のフレーム）は扱わないので、接点として表現できる出力が無いため —— コイルの励磁状態そのもの（「ON コマンド送信中」の意味を持たせる）が唯一の観測対象になる。`RelayDefinition.contacts` が空配列でも `evaluateCoil()` によるコイル判定・極性検出・プロパティパネルの表示は今までどおり効く（§5.3・§8.3）。`verified: false`（特定製品のカタログとは未照合）。
