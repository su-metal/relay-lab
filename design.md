...
| **同じ辺どうし（右 → 右）** | 出口の高さのまま相手の真横まで走り、そこから折れて相手の辺へ回り込む | そもそも中点を使わない。**走行の高さは出口で決まる** |

とくに後者が効く。電源のレールから複数の負荷へ渡す配線は「電源の右辺 → 各負荷の右辺」に
なりがちで、**同じ端子から出る線の走行が全部同じ高さに立ち、ピクセル単位で完全に重なる。**
...
| **相手に背を向けて出る配線（回り込む形）は対象外** | 走行が出口側ではなく相手側の座標に立ち、幹線の見立てが変わる。数も少ない |
