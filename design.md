...
src/
  ...
  components/
    circuit/
      ...
      ladder-image.ts            # ラダー図を PNG / SVG で保存（DOM 側だけ・§8.15）
  ...
  circuit/
    ...
    definitions/
      ...
      diodes.ts
      terminals.ts
      lighting-system.ts         # 実機の調光システムの機器（§4.15〜§4.17）
      av-equipment.ts            # プロジェクター・VPコントローラー・モニター・スクリーン（§4.19）
      __tests__/
        ...
        av-equipment.test.ts     # AV機器の補助コイル・VPコントローラー・AC負荷の検証（§4.19・§5.21）
    persistence/
    ...
    adapter/
      ...
      ladder-svg.ts              # ラダー図 → 1 枚の SVG（画像で保存・§5.16・§8.15）
      __tests__/
        ...
        ladder-svg.test.ts       # 画像に端子番号・断り書きが載るか、はみ出さないか（§8.15）
...
type ComponentCategory =
  ...
  | "av"      // AV機器（プロジェクター・VPコントローラー・モニター・スクリーン・§4.19）。
              // 電気的には lamp（AC負荷）や relay（制御リレー）で、専用 kind は持たない
...
type ElectricalDefinition =
  ...
  | { kind: "analog-source";                    // 0–10V の調光出力（§5.17）
      ...
      digitalOutputs?: DigitalOutput[]          // オープンコレクタ出力（§5.22・Step 26）
...
// オープンコレクタ出力（§5.22・Step 26）。channels（電圧を出す）とは別の面 ——
// 動作中は signalTerminal と commonTerminal が実際に短絡する接点で、
// 駆動源は通信で受けた値（channelId の % が 0 より大きいか）
type DigitalOutput = {
  id: string
  channelId: string        // CommunicationBinding.channelId と同じ名前で対応
  signalTerminal: string
  commonTerminal: string
  normallyClosed?: boolean // 立てると判定を反転（NO/NC の対を 2 エントリで表す）
  label?: string
}
```
...
type RelayDefinition = {
  coil?: {                    // 省略可能。カットリレー・操作卓のボタンには実機にコイルが無い
    voltage: number
    currentType: "DC" | "AC"
  ...
  auxCoils?: AuxCoil[]        // 主コイルとは独立に励磁する補助コイル（§4.19・§5.21）
...
// 主コイルとは独立に励磁する補助コイル（§4.19・§5.21）。
// 「複数の入力がそれぞれ独立に効く」機器のための形（昇降スクリーンの上昇/停止/下降）。
// `RelayContact` からは参照しない —— 接点を動かす駆動源としてではなく、
// 「その入力が今通電しているか」を見るためだけの宣言
type AuxCoil = {
  id: string
  label?: string
  voltage: number
...
**1 台が複数の独立した入力を持つ機器のために `auxCoils` を足した（Step 25）。** 昇降スクリーンの上昇・停止・下降は 3 本の無電圧接点入力で、どれが今通電しているかを個別に読みたい。単一の `coil` では 1 台につき 1 つの励磁状態しか持てず、`RelayContact.trigger` / `operationId` もアナログ量・人の操作の駆動源であって「外部から来た無電圧接点の通電そのもの」を表すものではない。`AuxCoil` は `coil` と同じ形（2 端子・極性）を持つが、**接点を動かさない** —— 見たいのは「この入力が今通電しているか」であって、実機に無い出力接点を作って動作を示すことはしない（CLAUDE.md 設計原則 6）。主コイルの有無とは独立で、昇降スクリーンのように主コイルを持たず `auxCoils` だけの機器もある。詳細は §4.19・§5.21。
...
この 10 定義は実型番を持たないため、**実端子番号も存在しない。** 実型番の端子番号と混同させないよう、次の扱いで統一する。
...
| `terminal-block-12p` | 汎用端子台 12P（全極短絡） | `1` 〜 `12` | すべて `generic` |
| `terminal-block-20p` | 汎用端子台 20P（全極短絡） | `1` 〜 `20` | すべて `generic` |

ダイオードのラベルを `1` / `2` にしないのは、**この部品では向きだけが情報**だから。番号を振ると「1 が入力」という無い決まりを読ませてしまう。端子台の通し番号は端子台に振られたものであって、型番ごとに決まった実端子番号ではないため `number` は持たせない（上段・下段に半分ずつ、左から昇順に配置する。6P なら上段 1・2・3 / 下段 4・5・6）。

**極数違い（6P / 12P / 20P）は実端子番号を持たない同じ抽象部品**（全極短絡・通し番号）なので、`definitions/terminals.ts` は極数と寸法を渡すビルダー関数 1 つで 3 定義とも組み立てる。実在メーカー品ではないため検証対象そのものが無く、極数を増やすだけなら定義ファイルを触らずに済む（CLAUDE.md 設計原則 2）。`TerminalBlockBody` は極数を `terminals.length` から読むので表示側の変更も無い。
...
**ON/OFF 出力（24–39）のうち、通信で受けて動くのは 33〜36 だけ（§5.22・Step 26）。** オープンコレクタは「動作したら GND へ落とす」接点で、駆動源（シーン記憶・フェーダー割付）は機器 1 台では決まらない（§4.16）。VP・スクリーン上昇/停止/下降（端子 33〜36）は操作卓からの通信を駆動源に持てるので `digitalOutputs` として実際の接点にした。残り（24–32・37–39）は対応する送信元がまだ無く、端子のまま。
...
#### 調光コントローラの ON/OFF 出力（24–39）は一部だけ繋がった

16 回路それぞれの駆動源（シーン記憶・フェーダー割付）は操作卓の内部ロジックで、機器 1 台では決まらない。**VP・スクリーン上昇/停止/下降（端子 33〜36）だけは、操作卓の通信を駆動源に持てるので Step 26 で `digitalOutputs`（§5.22）として実際に繋いだ。** 残り（24–32・37–39）は対応する送信元がまだ無く、端子のまま。
...
#### 送るのはフェーダー・照明スイッチ・VP電源・スクリーン上昇/停止/下降

電源ボタンは送らない。こちらは自分の無電圧接点（4-5-6）とオープンコレクタ出力（2・3）を動かすもので、コントローラ側の割り当て（端子 32・38・39）は別の話。

**VP電源・スクリーン上昇/停止/下降は送るしかない（Step 25 で追加）。** 電源ボタンと違い、この 4 つはコンソール前面パネルのボタンではあるがコンソールパネル接続端子（端子 1〜15）には現れず、コントローラの端子 33・34・35・36（VPランプ / スクリーン上昇/停止/下降 の ON/OFF信号出力）として初めて端子に出てくる。だから `operations` には足すが `contacts` には入れず、`communication.transmits` にだけ載せる —— 照明スイッチ 1–8 とまったく同じ形。

**コントローラ側で受ける先も Step 26 で繋がった。** `analog-source` に接点を持たせる構造判断（`digitalOutputs`・§5.22）を足し、この 4 つの信号は送られるだけでなく、コントローラの端子 33・34・35・36 を実際に GND へ落とす接点として動かす（§4.15）。
...
### 4.19 AV 機器（`definitions/av-equipment.ts`・Step 25 で確定）

調光コントローラ（§4.15）が持つ端子 32・34〜36・38・39（電源 ON/OFF 出力・昇降 上昇/停止/下降出力・電源制御 NO/NC）は、Step 21 の時点では「端子として出すが、まだ接点としては働かない」置き場所だった。Step 25 でその**先につながる実機**（プロジェクター・モニター・電動スクリーン）と、両者を仲介する VP コントローラーを追加した。

**Step 25 の時点では、調光コントローラ・操作卓が持つ 32〜39 番の出力は何も駆動していなかった。** Step 26 で `analog-source` に接点を持たせる構造判断（`digitalOutputs`・§5.22）を足し、VP・スクリーン上昇/停止/下降（端子 33〜36）だけ実際に駆動できるようにした。**モニターだけは別経路。** 社内仕様書（ver.1.1）のコンソール前面パネルにモニター専用ボタンの記載が無いため、新しい操作子は作らず、操作卓が実機どおり既に持つ電源接点（無電圧接点 4-5-6・オープンコレクタ出力 2-3-9・§4.16）を外部リレーの駆動源にする（ユーザー判断）。端子 32・37・38・39 は対応する送信元がまだ無く、引き続き端子のまま。

**実際の配線例。** プロジェクター・VP コントローラー・電動スクリーンは「操作卓 → 調光コントローラ（端子 33〜36）→ 実機」の経路（`av-equipment.test.ts` の「調光操作卓 → コントローラ → VP・スクリーンが実際に動く」）、モニターは「操作卓の電源接点 → 外部リレー（コイルは DC24V 等の外部電源から）→ モニターの AC100V ライン」の経路（同ファイルの「調光操作卓の電源ボタンでモニターを連動させる」）で、どちらもテストと同じ形で盤に組める。

#### プロジェクター（Panasonic PT-VX430J）

利用者から提供された Panasonic『液晶プロジェクター PT-VX430J』仕様書（VX430J_STM_01・2021/01/14 作成・1/8〜8/8）で確認した事実は次の 3 点。

- 使用電源は AC100V 50Hz/60Hz（■機器仕様）
- 接続端子は HDMI・コンピューター(RGB)・ビデオ・音声・**シリアル入力端子（D-Sub 9P・メス型・RS-232C 準拠、外部制御用）**・LAN・**USB A 端子（タイプA・無線モジュール WML100J / AJ-WM50GT 用の給電あり）**・USB B 端子のみ（■接続端子リスト・2/8 ページ）。**無電圧接点で開閉できる制御端子は無い**

この盤の他の実機（調光操作卓・調光コントローラ）と違い、PT-VX430J は電源 ON/OFF をリレー回路へ直接組み込める形で持っていない。**VP コントローラーはこの実機の USB A 端子から給電されて動作する**（ユーザー判断：無線モジュール用の給電端子であることを根拠にした一般化）。
...
PT-VX430J のように無電圧接点の制御端子を持たないプロジェクターへ、調光操作卓・調光コントローラ等の無電圧接点からの ON/OFF をシリアルコマンドへ変換して送る、市販の RS-232C 接続ボックス相当を一般化した部品。**実機の製品ではなく本アプリのための仮の中継機器**であることを明記したうえで追加した（ユーザー判断）。

**自分の動作用電源は外部の電源部品からではなく、プロジェクター自身の USB 出力から取る**（ユーザー判断）。コイル 1 個（`VBUS` / `CTRL`）と、プロジェクターのシリアル入力端子へつなぐための飾りの端子（`SERIAL`）、プロジェクターの USB GND へ直結する `GND` を持つ。コイルの正側 `VBUS` をプロジェクターの `VBUS` へ、負側 `CTRL` を「操作卓等の無電圧接点を経由して」プロジェクターの USB GND（`GND` 端子経由でもよい）へ配線する形にすることで、**プロジェクターが給電されていて、かつ操作卓の接点が閉じている**ときだけコイルが励磁する —— § 5.20 の `ac-dc-power-supply` 判定がそのまま「プロジェクターの給電」の条件を担う。実機なら電源系統と制御系統を別に持つところを 2 端子（`VBUS`/`CTRL`）に畳んでいるが、このシミュレーターが見せたい 2 条件（給電の有無・操作卓の接点の開閉）はこの形で過不足なく表せる。

**接点は 1 個も持たない。** 実機の変換の中身（RS-232C のフレーム）は扱わないので、接点として表現できる出力が無いため —— コイルの励磁状態そのもの（「ON コマンド送信中」の意味を持たせる）が唯一の観測対象になる。`RelayDefinition.contacts` が空配列でも `evaluateCoil()` によるコイル判定・極性検出・プロパティパネルの表示は今までどおり効く（§5.3・§8.3）。`verified: false`（特定製品のカタログとは未照合）。

**プロジェクターの投影 ON/OFF が盤面から視覚的に読めるよう、ノードの見た目もこのコイルの励磁を見せる**（`AvBody.tsx` の `VpControllerVisual`）。プロジェクター本体（`kind: "ac-dc-power-supply"`）は投影 ON/OFF の概念を電気的に持たないため専用の絵を持たせられない ——代わりにこの VP コントローラーのノードへ、`RelayBody` と同じコイル記号（励磁で色が変わる）とキャプションを出す。励磁は「操作卓のボタンを倒している間」の状態であり投影の持続を保証するものではないため、キャプションは「ON コマンド送信中 / 未送信」という言い方に留め、「投影中」のような実際の状態を主張する表現は使わない。

#### モニター（汎用）

「型番は問わない、一般的な業務用モニターの電源 ON/OFF ができれば十分」というユーザー判断のもと、`ac100vLamp`（§4.5）と同じ形の AC100V 負荷として追加した。**モニター自身に制御端子は持たせない** —— 電源 ON/OFF は外部のリレー／電磁接触器の接点で AC100V ラインを直接断続する構成とする（ユーザー判断）。カテゴリだけ `"av"` にして、プロジェクター・スクリーンと並べて探せるようにしてある（`category` は表示都合だけ・§3.1）。`verified: false`。

#### 電動スクリーン（汎用・§5.21 の補助コイルを初めて使う部品）

多くの電動スクリーンが持つ「COM・UP・DOWN・STOP の無電圧接点入力 4 端子＋モーター用 AC100V」という一般的な構成を採った（ユーザー判断：「反応・動作が分かればよい、コントローラーから出た信号でリレーが反応する感じ」）。上昇・停止・下降は**互いに独立した 3 本の入力**で、どれか 1 本が今通電しているかを個別に読みたい —— 単一の `coil` では 1 台につき 1 つの励磁状態しか持てないため、§3.2・§5.21 で導入した `auxCoils`（`up` / `stop` / `down` の 3 個、いずれも COM を負側に共有）で表す。主コイル（`coil`）は持たない。

接点も持たない。上昇・停止・下降の反応はプロパティパネルの「コイル」節に並ぶ各補助コイルの励磁状態で確認する（§8.3）——実機に無い出力接点を作って動作を示すことはしない（CLAUDE.md 設計原則 6）。モーター駆動用の AC100V 端子は物理的な実在を示すためだけに置き、実際にモーターが回るかどうかは判定しない。

**上昇・下降・停止が同時に入力された場合のインターロックは実機の仕様が不明のため持たせていない**（§6 既知の制約）。3 系統は完全に独立して評価されるので、同時に上昇と下降を通電させても両方が「励磁中」と出る。`verified: false`。
...
#### 画像にするときも図を描き直す（`adapter/ladder-svg.ts`）

画像で保存する（§8.15）ときは、**画面の DOM を写し取るのではなく `LadderDiagram` から SVG を描き直す。** 理由は 2 つ。

1. **スクロールの外まで出る。** 画面のラダー図は横にも縦にもスクロールするので、見えている部分だけを写すと段が途中で切れる
2. **純粋関数として検証できる。** DOM も React も読まないので、端子番号・断り書き・段の数が絵に載っているかを Vitest で確かめられる

外部ライブラリ（html2canvas 系）を足さないのも同じ理由で、あれは CSS の描き方に結果が左右される。

図の骨組みは画面（§8.15）と**同じ寸法**で組む —— 図記号の中心は上から 30px、要素の高さは 58px、並列の左右に 12px。画面が CSS で引いている横線と切れ目を、こちらは座標で引き直すだけ。違いは 3 つだけ。

| | 画面（CSS） | 画像（SVG） |
|---|---|---|
| 横線 | 行の擬似要素 | `<line>` を先に引き、図記号の下地で切れ目を作る |
| 母線 | 各段の枠線を縦に積む | 1 本の `<line>`（段の間で切れない） |
| 色 | `var(--text)` | **16 進の実値**（アプリの外で開くと CSS 変数はどこにも解決されない） |

文字幅は DOM を読めないので**全角 1.0em・半角 0.62em で見積もる。** 少し広めに取る —— 狭く見積もると呼び名が隣の接点へ被るが、広い分には間延びするだけで読めなくならない。

**生成日時を図の右上に入れる。** ラダー図は保存対象ではない派生物（CLAUDE.md 設計原則 10）だが、画像にした瞬間から実配線と食い違い得る。いつの配線から起こした図かが絵の中に無いと、古い画像を見て配線を直す事故が起きる。**時計はここで読まない**（設計原則 1 と同じ約束で `generatedAt` として受け取る）。

断り書き（ダイオード・調光を図に出していないこと、電源を 1 組の母線に束ねたこと）も**必ず画像に載せる。** 画面には出て画像には出ないと、絵だけを見た人が誤読する。段が 1 本も無いときも「段が無い」と描いた絵を返す —— 空の絵は「書き出しに失敗した」のか「出す段が無い」のか受け取った側に分からない。
...
**1 台が複数の独立した入力を持つ機器のための評価。** 昇降スクリーンの上昇・停止・下降のように、単一の `coil` では表せない「それぞれ独立に通電したかを読みたい入力」が実機にはある。`evaluateRelays()`（`engine/simulate.ts`）は主コイルの評価とは別に、`electrical.relay.auxCoils` の各要素を `evaluateCoil()`（§5.3）へそのまま渡し、2 端子間の電位差だけで励磁を判定する。

**接点にもネットの形にも触れない。** 補助コイルは主コイルとまったく同じ理由で union されない負荷であり（§5.2）、`RelayContact` から参照されることも無い。結果は `SimulationResult.auxCoilsEnergized`（componentId → 励磁している補助コイル ID の集合）としてだけ返り、`energizedRelays`（接点が切り替わっている・§3.4）とは別の軸に置く。1 台の中で「上昇だけ励磁」「上昇と下降が同時に励磁」のような状態をそのまま表せる。

**極性の誤りも主コイルと同じ規則で警告する。** `polarity: "strict"` の補助コイルを逆に繋ぐと `coil-polarity-reversed` 警告が出る（§5.7）。主コイルの警告メッセージが「コイルの」と言う箇所を、補助コイルでは `label`（"上昇" 等）に差し替えるだけで、判定ロジックは完全に共有している。

**表示はプロパティパネルの「コイル」節に追加する形にした（§8.3）。** 実機に無い出力接点を新設して動作を示すのではなく、`auxCoils` それぞれの励磁状態を主コイルと並べて一覧できるようにする —— 見たいのは「この入力が今通電しているか」そのもので、それを示すための接点を機器に足すのは CLAUDE.md 設計原則 6 に反する（§4.19 の電動スクリーンが最初の利用例）。

### 5.22 オープンコレクタ出力（`AnalogSourceDefinition.digitalOutputs`・Step 26 で追加）

**通信で受けた値が、実際に閉じる接点になる。** 調光コントローラの端子 24〜39 は「動作したら GND へ落とす」オープンコレクタで、Step 21・24・25 では端子として置くだけに留めていた（§6 旧項目）。Step 26 で、VP・スクリーン上昇/停止/下降（端子 33〜36）だけこれを実際の接点にした。

**`channels`（0–10V の電圧出力）とは別の面に持たせる。** `AnalogOutputChannel` は「信号端子とコモンを union しない」原則（CLAUDE.md 設計原則 3）の対象そのものだが、オープンコレクタは物理的にまったく逆で、**動作中は実際に短絡する接点**である。同じ `channels` 配列に混ぜると「union しない」の例外がその場その場の型番判断になってしまうため、`digitalOutputs?: readonly DigitalOutput[]` という新しい面を `analog-source` に足した。`DigitalOutput` は `id`・`channelId`・`signalTerminal`・`commonTerminal`・`normallyClosed?` を持ち、`channelId` は `CommunicationBinding.channelId` と同じ名前で対応させる（実装は `dimmingController16ch` で `channelId` を端子番号そのものにしてあり、1–16 の `channels` の ID と衝突しない）。

**「動作しているか」は通信の % がそのまま二値の駆動源になる。** `engine/graph.ts` の `digitalOutputPairs()` は `resolveCommunication()` が返す `CommunicatedLevels`（componentId → channelId → 0–100%）を読み、対象チャンネルの % が 0 より大きければ `[signalTerminal, commonTerminal]` を導通ペアとして返す（`normallyClosed` を立てると判定を反転する ―― 実機の「電源制御 ノーマルオープン／ノーマルクローズ」のように、同じ 1 つの状態から NO・NC の対を出す機器を、2 つの `DigitalOutput` エントリで表せる）。電圧への変換（`analog.ts`）は経由しない ―― オープンコレクタは V の値を持たない、ただの二値の接点だから。

**通信の解決結果を収束ループの中の `buildNets()` へも渡す。** `communication.levels` 自体は `simulate()` のループの外で 1 回だけ解く既存の割り切り（§5.19）のままだが、`conductingPairs()`／`buildNets()` に新しい引数 `communicatedLevels?: CommunicatedLevels` を足し、ループの各反復で `operatedContacts` と同じ位置に渡す。値そのものは 1 回の `simulate()` の中で変わらないので、反復のたびに解き直す必要は無い。

**「通信はネットの分割に触れない」という §5.19 の原則を破っているように見えるが、対象が違う。** §5.19 が言う「触れない」は 0–10V の信号端子（`channels`）についての話で、そちらは今も union しない。オープンコレクタ出力は実機でも文字どおりトランジスタが GND へ短絡する接点であり、`RelayContact`（コイルで動く接点）・`auxCoils`（励磁を読むだけの疑似コイル）に続く**駆動源が違う接点の 3 つ目の形**というのが正しい理解 ―― 駆動源が「人が押す」でも「コイルが引く」でもなく「通信で受けた値」なだけで、閉じたら union するという規則自体は他の接点とまったく同じ。
...
9. **電動スクリーンの上昇・停止・下降にインターロックは無い（§4.19・§5.21）。** 3 系統の補助コイルは完全に独立して評価されるため、上昇と下降を同時に通電させると両方が「励磁中」と出る。実機がどちらを優先するか（あるいは両方を無効にするか）の仕様が不明なため、あえて何も足していない。
...
**部品交換（Step 9 で確定）。** 名前欄のすぐ下に「部品交換」のセレクトを置く。選択肢は `componentDefinitions` を現在の部品と**同じカテゴリ**（`category`）かつ自分自身を除いたものに絞り込む。候補が無いカテゴリ（電源・ランプ・ダイオードは現状 1 種類ずつ）ではセレクト自体を出さない。選ぶと即座に `replaceComponentDefinition()` を呼んで交換し、セレクトは常に空文字（プレースホルダー「交換先を選択…」）へ戻す — 交換後は候補一覧そのものが変わる（今の部品が新しい候補に加わり、選んだ部品が外れる）ため、選んだ値を保持し続ける意味が無い。**端子台は 6P / 12P / 20P の 3 種になったので、ここで極数を選び直せる**（`category: "terminal"` を共有するだけで、定義側は何も足していない）。
...
**次の 3 つは、上の仕組みが何も動かせない、または動かしても足りない。**

| 形 | `getSmoothStepPath` が描くもの | なぜ動かせない／足りないか |
|---|---|---|
| 向かい合っていて高さも同じ（右 → 左） | 直線 1 本 | 幹線の長さが 0。動かす対象そのものが無い |
| **向かい合っていない**（右 → 右、辺が直交） | 決まった高さをまっすぐ走り、そこから折れて相手の辺へ回り込む | そもそも中点を使わない |
| **上下の端子どうしで、向かい合って相手のほうへ進む形** | 端子ごとの高さのまま相手の真横まで伸びる 2 本の縦の助走 → その間を繋ぐ短い横線（中点） | `getDirection`（React Flow 内部）は上下の端子では常に「主方向は縦」と決め打つ。動かせる中点は 2 本の助走の**あいだの短い区間でしかなく**、助走そのもの（端子の x に貼り付いたまま、両端の間をほぼ埋める長さになりうる）は動かない |

とくに 2 番目が効く。電源のレールと複数の負荷を結ぶ配線は「右辺 → 右辺」になりがちで、
**走行が全部同じ高さに立ち、ピクセル単位で完全に重なる。**

**3 番目は左右の端子どうしでは起きない。** 左右の端子は `getDirection` が「主方向は横」と
決め打つので、動かせる中点＝縦の助走のほうが長い区間になり、`centerX` で正しく避けられる。
高さがぴったり揃って直線に潰れたとき（1 番目）だけ例外的にこちらへ回す。
...
| 上下の端子どうしで、高さがずれたまま間の部品を跨ぐ線 | 端子の x に貼り付いた縦の助走が部品を突っ切る。中点（横線の高さ）をどれだけ動かしても助走そのものは動かないので、`straightRunPath` の走行として扱わないと避けられない（上の表を参照） |
...
| 形 | 上限（`Trunk.detour`） |
|---|---|
| 左右の端子どうしで、相手を向いて出る（幹線が両端の間に立つ） | `room` と同じ。ここから出すと経路が折り返す |
| **相手に背を向けて出る（回り込み）** | `MAX_DETOUR`。幹線は両端の外を通る迂回線そのもので、どこへ置いても形が崩れない |
| 自前で組む走行（`straightRunPath`。上下の端子どうしで相手を向いて出る形はすべてこちら） | `MAX_DETOUR`。ただし走行が `MIN_JOG_RUN` 未満なら 0（迂回の折れが収まらない） |
...
**1 行目は左右の端子どうしに限る。** 上下の端子どうしで相手を向いて出る形（`alongAxis`）は、
高さがずれていると `room`（中点の可動域）を上限にしても助走そのものは避けられない
（上の表を参照）。そのため `trunkOf` はこの形を「相手を向いて出る」側ではなく
`straightRunPath` の走行として分類し、`MAX_DETOUR` まで動かす。
...
見出しの右（閉じるボタンの手前）に `PNG で保存` と `SVG で保存` を置く。**操作バーには出さない** —— ラダー図を開いていないと「何を保存するのか」が決まらない。段が 1 本も無いときは絵にする中身が無いので押せない。

| | 用途 |
|---|---|
| PNG | 資料や報告書に貼る。等倍だと 10.5px の端子番号が潰れるので **2 倍で焼く** |
| SVG | 拡大しても崩れない。後から図面ソフトで開いて注記を足せる |

絵を作るのは `adapter/ladder-svg.ts`（§5.16）で、ここ（`ladder-image.ts`）が持つのはブラウザ側の仕事だけ —— Blob にする、`<img>` と `<canvas>` で PNG に焼く、`<a download>` を押す。`useDocumentPersistence.exportToFile()` と同じ役割分担（書式は純粋関数、ダウンロード操作は DOM 側）。ファイル名は `relay-lab-ladder-20260820-1530.png`（`circuitFileName()` と同じ作りで日本語を含めない）。

PNG は `<img>` の読み込みと `toBlob()` を通るので失敗しうる。**失敗はダイアログ内に出す** —— 押しても何も起きないのが一番困るので、SVG なら同じ図を書き出せることまで言う。
...
| 25 | AV 機器（プロジェクター・VP コントローラー・モニター・電動スクリーン）＋補助コイル | 端子 32・34〜36・38・39 の先につながる実機をパレットから置け、電動スクリーンの上昇/停止/下降が独立した補助コイル（`auxCoils`）として励磁する。プロジェクターは AC100V で USB 出力（VBUS/GND）に別の DC 電位が立ち、VP コントローラーはその給電と操作卓の接点の両方が揃ったときだけ励磁する（§4.19・§5.20・§5.21） |
| 26 | オープンコレクタ出力（`digitalOutputs`）＋モニター・スクリーンの見た目 | 調光コントローラの端子 33〜36 が、操作卓の VP電源・スクリーン上昇/停止/下降ボタンで実際に GND へ落ちる接点として動く。モニターは操作卓の実在する電源接点で外部リレーを介して連動できる。モニターは点灯すると画面にプレースホルダーの絵、電動スクリーンは上昇/下降中に生地が流れるアニメーションを出す（§5.22） |
