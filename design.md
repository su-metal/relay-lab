...
**`kind` を増やすのは最後の手段。** タイマーは `relay` の `delay`、調光ランプは `lamp` の `dimming` で表しており、どちらも `kind` を増やしていない（CLAUDE.md 設計原則 7）。`analog-source` だけが増えたのは、既存のどれにも寄せられなかったから —— 電位を配る `power` でも、電位差を受ける `lamp` でもなく、**基準に対する電圧値を出す**という別の振る舞いだった。
...
**ON/OFF 出力（24–39）と通信線（22・23）は端子として出すだけ。** オープンコレクタは「動作したら GND へ落とす」接点で、アナログ量から接点を動かす仕組みが要る（次スコープ）。通信は「電位がどこまで届くか」で判定するこのエンジンでは中身に意味が出ない（§6）。
...
`simulate()` が呼ぶのは**収束したあとに 1 回だけ。** 調光レベルは接点を動かさない —— レベルが変わってもリレーは励磁せず、ネットの形も変わらないので、反復の中で解き直す理由が無い。ダイオードの伝搬（§5.4）が `computeNetStates()` の中にあるのとはここが違う。
...
3. **電圧・電流・消費電力の数値は扱わない（調光の 0–10V を除く）。** 電源まわりは導通の有無のみで、定格電圧の不一致（DC24V ランプに AC100V など）は MVP では検出しない。
   ...
   **調光で扱わないもの。** 時間をかけて明るさが変わるフェード（時間の土台は §5.13 にあるが本スコープでは即座に変わる）、RS-485 / DMX などの通信、位相制御（トライアック）・力率・消費電力の数値、出力段のインピーダンス（同じネットを複数の調光出力が駆動したら低いほうが勝つ、という 1 つの規則で近似する）。
