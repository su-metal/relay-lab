...
| 対象 | 表示 |
|---|---|
| 向きの決まった通電線 | 線の上を**背景色の切れ目**が流れる（`WireEdge.module.css` の `.flow`）。3px の切れ目・16px 周期 |
| 自己保持の紫（§5.9） | 線自身の破線の流れる向きを、電流の向きに合わせる |
| 凡例 | 実行中の 3 番目に「電流の向き」。**色ではなく動きの見本**なので、キャンバスと同じ周期で背景を流す |

切れ目を線の上に重ねるのは、**下の線が緑でも紫でも同じ 1 つのルールで済ませるため。** 色を足すと §5.6 の色の意味を上書きしてしまう。
