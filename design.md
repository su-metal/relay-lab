...
**コントローラ側で受ける先はまだ無い。** 端子 24–39（開放コレクタの ON/OFF 出力）を実際の接点として働かせるには `analog-source` に接点を持たせる構造判断が要り、Step 21・24 に続いてなお次スコープに残る（§6）。したがって今のところ、この 4 つの信号は送られるところまでは動くが、コントローラの端子 33・34・35・36 は「端子として出ているだけでまだ何も駆動しない」（§4.15）ままで変わらない。
...
**この盤の他の機器（調光コントローラ・操作卓）が持つ 32〜39 番の出力そのものは、今回もまだ何も駆動していない。** 開放コレクタ出力を実際の接点として働かせるには `analog-source` に接点を持たせる構造判断が要り、これは Step 24 に続いてなお次スコープに残る（§6）。したがってここで追加した 4 機器は、現状は外部の電源と接点（あるいは他のリレー・電磁接触器の出力）で個別に駆動して使う。
